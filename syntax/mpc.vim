...
syntax match mpc_project
  \ "project\(([A-Za-z0-9_\*]\+)\)\?\(:\s\+[A-Za-z0-9_\*, \\]\+\)\?"
  \ transparent contains=mpc_keyword,mpc_break_line skipwhite nextgroup=mpc_block

syntax region mpc_block start="{" end="}" contains=mpc_comment,mpc_setting_name skipwhite

syntax match mpc_setting_name "[A-Za-z0-9_]\+"
  \ contained contains=mpc_keyword skipwhite nextgroup=mpc_setting_op,mpc_block
syntax match mpc_setting_op "[+-]\?=" contained skipwhite nextgroup=mpc_setting_value
syntax region mpc_setting_value start=" " end="[^\\]\?$"
  \ contained skipwhite contains=mpc_etting_op,mpc_variable,mpc_comment,mpc_break_line

" syntax match mpc_custom "[A-Za-z0-9_]\+" contained skipwhite nextgroup=mpc_block
...
highlight default link mpc_verbatim Comment
...
highlight default link mpc_setting_value String
highlight default link mpc_error Error
highlight default link mpc_keyword Keyword
highlight default link mpc_custom Identifier
